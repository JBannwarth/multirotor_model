%PLOTPARAMETERVARIATIONS Plot position hold metrics
%   Written: 2021/03/29, J.X.J. Bannwarth

%% Clean-up
clearvars -except varied filenameOut
close all

%% Set-up
project     = simulinkproject;
projectRoot = project.RootFolder;
resultDir   = fullfile( projectRoot, 'data_results', 'poshold_variation' );
...
if ~exist( 'filenameOut', 'var' )
    filenameOut = [ 'parameter_variation_' varied ];
end
...
for ii = 1:length( testCases )
    files =  dir( fullfile( resultDir, [ testCases{ii} '*' varied '*.mat' ] ) );
...
for ii = 1:length( simsOut )
    for jj = 1:length( simsOut{ii} )
        ...
        M.identifier(idx) = sprintf( '%s_%04.1f', testCases{ii}, M.xVal(idx) );
        M.group(idx) = sprintf( '%s_%s', testCases{ii}, ...
             replace( num2str( round( simsIn{ii}(jj).getVariable('param').(varied), 2, 'significant' ) ) , '.', '') ) ;
