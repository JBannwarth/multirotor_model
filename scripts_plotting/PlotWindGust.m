...
resultDir   = fullfile( projectRoot, 'data_results', 'gust_full' );
tStart = 100; % [s] time the gust starts
gustLen = 30;  % [s] length of flight to analyse
...
% Fill table
deltaT = 0.2;
dataEx = zeros(gustLen/deltaT + 1, 1+(5)*length(simsOut{1}));
idx = 0;
header = cell( 1+(5)*length(simsOut{1}), 1 );
header{1} = 't';
tags = { 'BL', 'VT' };
for ii = 1:length( simsOut )
    for jj = 1:length( simsOut{ii} )
        ...
        M.xVal(idx) = mean(simsIn{ii}(jj).getVariable('stepSize'));
        M.xLabel(idx) = num2str( M.xVal(idx) );
        M.identifier(idx) = sprintf( '%s%03.1f', tags{ii}, M.xVal(idx) );
        ...
        pwmTrue = 1075 + squeeze( pwm.Data )' .* (1950-1075);
        pwmMin = min( pwmTrue, [], 2 );
        pwmMax = max( pwmTrue, [], 2 );
        dataEx(:,1) = tResampleOut';
        dataEx(:,(2:6)+(idx-1)*5) = [ x.Data z.Data eul(:,2) pwmMin pwmMax ];
        header((2:6)+(idx-1)*5) = cellfun( @(x)(sprintf( '%s%s', x, replace(M.identifier(idx),'.',''))), { 'x', 'z', 'pitch', 'pwmMin', 'pwmMax' } , 'UniformOutput', false);
    end
end
...
filenameOutEx = fullfile( projectRoot, 'work', 'output', [filenameOut '_response.csv' ] );
