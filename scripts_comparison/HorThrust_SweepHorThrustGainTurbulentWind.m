...
for i = 1:length( windFiles )
    load( fullfile( inFolder, windFiles{i} ), 'windInput' )
...
if exist( fullfile( projectRoot, 'work', [opFile '.mat'] ), 'file' ) == 2
    ...
    opspec = operspec( model, [length(windInputs) 1] );
    ...
    for i = 1:length(opspec)
        ...
        for j = 1:size(inputSpecs,1)
            inputInd = opspec(j,1).getInputIndex( [ model '/' inputSpecs{j,1} ] );
...
for i = 1:length( windInputs )
    for j = 1:length( gainVals )
        ...
        simIn(i,j) = setVariable( simIn(i,j), 'windInput', windInputs(i) );
        simIn(i,j) = setVariable( simIn(i,j), 'op', opts(i) );
    end
end
...
simOutputs = reshape( simOutputs, [length(windInputs),length(gainVals)] );
...
axMean.X = subplot(4,1,1); hold on; grid on; ylabel('$x_\mathrm{mean}$ (m)'); ylim([-0.0015 0.0015]); xlim([-inf 6])
axMean.Y = subplot(4,1,2); hold on; grid on; ylabel('$y_\mathrm{mean}$ (m)'); ylim([-0.0015 0.0015]); xlim([-inf 6])
axMean.Z = subplot(4,1,3); hold on; grid on; ylabel('$z_\mathrm{mean}$ (m)'); ylim([-0.0015 0.0015]); xlim([-inf 6])
axMean.N = subplot(4,1,4); hold on; grid on; ylabel('$|\mathbf{x}|_\mathrm{mean}$ (m)'); ylim([0 0.017]); xlim([-inf 6]); xlabel('$U_{x,\mathrm{mean}}$ (m/s)')
axStd.f = figure('Name', 'Position standard dev.');
axStd.X = subplot(4,1,1); hold on; grid on; ylabel('$x_\mathrm{std}$ (m)'); ylim([0 0.017]); xlim([-inf 6])
axStd.Y = subplot(4,1,2); hold on; grid on; ylabel('$y_\mathrm{std}$ (m)'); ylim([0 0.017]); xlim([-inf 6])
axStd.Z = subplot(4,1,3); hold on; grid on; ylabel('$z_\mathrm{std}$ (m)'); ylim([0 0.017]); xlim([-inf 6])
axStd.N = subplot(4,1,4); hold on; grid on; ylabel('$|\mathbf{x}|_\mathrm{std}$ (m)'); ylim([0 inf]); xlim([-inf 6]); xlabel('$U_{x,\mathrm{mean}}$ (m/s)')
...
for i = 1:length( gainVals )
    ...
    plot( ULin(:,1), meanPos.x(:,i), 'LineStyle', lineStyle{i}, 'Color', cols(i,:) )
    set(axMean.f, 'currentaxes', axMean.Y);
    plot( ULin(:,1), meanPos.y(:,i), 'LineStyle', lineStyle{i}, 'Color', cols(i,:) )
    set(axMean.f, 'currentaxes', axMean.Z);
    plot( ULin(:,1), meanPos.z(:,i), 'LineStyle', lineStyle{i}, 'Color', cols(i,:) )
    set(axMean.f, 'currentaxes', axMean.N);
    plot( ULin(:,1), meanPos.N(:,i), 'LineStyle', lineStyle{i}, 'Color', cols(i,:) )
    
    % Std
    set(0, 'currentfigure', axStd.f);
    set(axStd.f, 'currentaxes', axStd.X);
    plot( ULin(:,1), stdPos.x(:,i), 'LineStyle', lineStyle{i}, 'Color', cols(i,:) )
    set(axStd.f, 'currentaxes', axStd.Y);
    plot( ULin(:,1), stdPos.y(:,i), 'LineStyle', lineStyle{i}, 'Color', cols(i,:) )
    set(axStd.f, 'currentaxes', axStd.Z);
    plot( ULin(:,1), stdPos.z(:,i), 'LineStyle', lineStyle{i}, 'Color', cols(i,:) )
    set(axStd.f, 'currentaxes', axStd.N);
    plot( ULin(:,1), stdPos.N(:,i), 'LineStyle', lineStyle{i}, 'Color', cols(i,:) )
end
...
SetFigProp( [12,20], 10 )
MatlabToLatexEps( fullfile( projectRoot, 'work', 'PropGain_Mean' ) )

set(0, 'currentfigure', axStd.f); set(axStd.f, 'currentaxes', axStd.X);
legend( legendStr, 'Orientation', 'vertical', 'Location', 'best' )
SetFigProp( [12,20], 10 )
MatlabToLatexEps( fullfile( projectRoot, 'work', 'PropGain_Std' ) )
